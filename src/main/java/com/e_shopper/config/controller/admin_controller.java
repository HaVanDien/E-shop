...
@RequestMapping(value = "/cusListAd", method = RequestMethod.GET)
    public String getlisad(ModelMap mm, HttpSession session) {
        if (session.getAttribute("admin") != null) {
            // get all unprocessed orders 
            List<customer> custList = dao_admin.getcustList();
            mm.put("custList", custList);
    ...
    public String addProduct(ModelMap mm, HttpSession session,
            ...
            @RequestParam(value = "prod_price") String pro_price,
            @RequestParam(value = "prod_stock") String pro_stock,
            @RequestParam(value = "prod_category") String pro_category,
        ...
        if (session.getAttribute("admin") != null) {
            ...
            if (valiForm.validateName(pro_name) > 0 || valiAd.valiProdNum(pro_price) > 0
                    || valiAd.valiProdNum(pro_stock) > 0
                    || valiForm.validateAddress(prodimgLink) > 0 || valiForm.validateName(pro_category) > 0 || prodimgLink == null) {
                if (valiForm.validateName(pro_name) > 0) {
                    mm.put("proNamVali", valiForm.validateName(pro_name));
                } else {
                    mm.put("pro_name", pro_name);
                }
                if (valiForm.validateName(pro_category) > 0) {
                    mm.put("proCateVali", valiForm.validateName(pro_category));
                } else {
                    mm.put("pro_category", pro_category);
                }
                if (valiAd.valiProdNum(pro_price) > 0) {
                    mm.put("proPricVali", valiAd.valiProdNum(pro_price));
                } else {
                ...
                // insert into data
                dao_admin.insertProd(pro_name, Integer.parseInt(pro_price), prodimgLink, Integer.parseInt(pro_stock),pro_category);
                // send view
    ...
    public String searchProd(ModelMap mm, @RequestParam(value = "searchProd") String inStr) {
     ...
     @RequestMapping(value = "/searchcust", method = RequestMethod.POST, params = {"searchOrd"})
    public String searchcust(ModelMap mm, @RequestParam(value = "searchOrd") String inStr) {
        // process inputed string
        ValidateForm valiF = new ValidateForm();
        String[] words = valiF.sepString(inStr);
        // get product from inputed string
