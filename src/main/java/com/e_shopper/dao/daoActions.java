...
@Override
    public List<product> getProListSport() {
        String Sport = "sport";
        String sql = "SELECT * FROM product WHERE product_category = '" + Sport + "'";
        List<product> proListSport = jdbcTemplate.query(sql, (ResultSet rs, int i) -> {
            product pro = new product();
            pro.setPro_id(rs.getInt("product_id"));
            pro.setPro_name(rs.getString("product_name"));
            pro.setPro_price(rs.getInt("product_price"));
    ...
    public List<customer> getcustList() {
        ...
        List<customer> cusList = jdbcTemplate.query(sql, (ResultSet rs, int i) -> {
            customer custom = new customer();
                    custom.setCustom_id(rs.getInt("custom_id"));
                    custom.setCustom_pass(rs.getString("custom_pass"));
                    custom.setCustom_email(rs.getString("custom_email"));
...
// update product table 
    ...
    @Override
    public void insertProd(String pro_name, int pro_price, String pro_img_link, int pro_stock,String pro_category) {
        String sql = "INSERT INTO `product` ('pro_id',`product_name`,`product_price`,`product_image_link`,`product_stock`,`product_category`)"
                + "VALUES ('"+50+"'," + pro_name + "'," + pro_price + ",'" + pro_img_link + "'," + pro_stock + ",'"+pro_category+"')";
        jdbcTemplate.update(sql);
    }
    
    @Override
    public void insertProd(int pro_id,String pro_name, int pro_price, String pro_img_link, int pro_stock,String pro_category) {
        String sql = "INSERT INTO `product` (`product_name`,`product_price`,`product_image_link`,`product_stock`,`product_category`)"
                + "VALUES (" + pro_name + "'," + pro_price + ",'" + pro_img_link + "'," + pro_stock + ",'"+pro_category+"')";
        jdbcTemplate.update(sql);
    }
    @Override
